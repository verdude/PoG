...
private:
	...
	float calc_vertical_location_in_jump(float milliseconds) {
		if (jmp_ctrl.jumping && !jmp_ctrl.grounded) {
			...
			float vertical_pos = jmp_ctrl.function_coefficient *
			                     (jmp_ctrl.jump_time - jmp_ctrl.zero_1) *
			                     (jmp_ctrl.jump_time - jmp_ctrl.zero_2);
...
public:
	...
	themainbro(float start = 0, float jump_duration = 500, float jump_height = -100)
	    : box(), xvel(), yvel(), xpos(), ypos(), rightImgs(), leftImgs(), direction('r'),
	      health(10), speed(300), keys(), frame(), totalFrames(), jmp_ctrl(),
	      max_jump_duration(jump_duration), jump_height(jump_height) {
	...
	Wrapper* getDefaultWrapper() {
		return direction == 'r' ? (rightImgs.size() > 0 ? rightImgs[0] : NULL)
		                        : (leftImgs.size() > 0 ? leftImgs[0] : NULL);
	}
