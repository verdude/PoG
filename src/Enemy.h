...
public:
	Enemy(SDL_Surface* img, int x, int y, int xVel, int yVel) {
		...
		yvel = yVel;
		ground = 0;
		// we set the clips for the image, so we can do animation, by blittin first one part of the
		// image, than the other one
		for (int i = 0; i < 2; i++) {
			clips[i].x = i * 40;
			clips[i].y = 0;
	...
	void Enemy::show(SDL_Surface* screen) {
		SDL_Rect tmp = {box.x - coord.x, box.y, 40,
		                40}; // calculate the relative coordinate, so if the Enemy at 120px
		                     // coordinate and the camera
		// at 100px coordinate, than we show it on 20px coordinate
		SDL_BlitSurface(image, &clips[(int)(frame + 0.5)], screen,
		                &tmp); // we just blit it. (int)(frame+0.5) is basically the round
	}
	...
	void Enemy::move(std::vector<std::vector<int>>& map) {
		int start = (baseclass::coord.x - (baseclass::coord.x % baseclass::TILE_SIZE)) /
		                baseclass::TILE_SIZE -
		            2;
		int end = (baseclass::coord.x + baseclass::coord.w +
		           (baseclass::TILE_SIZE -
		...
		bool falling = 0; // if this remains 0 until the end of the loops, than we fall (no ground
		                  // under the Enemy)
		for (int i = 0; i < map.size(); i++) // go through the map
			for (int j = start; j < end; j++) {
				...
				{
					if (destrect.y >=
					    box.y + box.h - 11) // and it'a the ground (so the tile is under the Enemy)
					{
